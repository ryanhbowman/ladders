...
@interface ZXingWidgetController ()<DecoderDelegate, CancelDelegate>
...
#endif
@synthesize delegate, soundToPlay;
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
  ...
  if (self) {
    ...
    //Add all readers
    MultiFormatReader* multireader = [[MultiFormatReader alloc] init];
    NSSet *readersSet = [[NSSet alloc ] initWithObjects:multireader, nil];
    [multireader release];
    self.readers = readersSet;
    [readersSet release];
  }
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  self.wantsFullScreenLayout = YES;
    ...
    self.overlayView.frame = CGRectMake(0, 0, size.width, size.height);
...
- (void)decoder:(Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
  ...
  // now, in a selector, call the delegate to give this overlay time to show the points
    [self performSelector:@selector(notifyDelegate:) withObject:twoDResult afterDelay:0.2];
  decoder.delegate = nil;
}

- (void)notifyDelegate:(TwoDDecoderResult *)twoDResult{
    if (!isStatusBarHidden) [[UIApplication sharedApplication] setStatusBarHidden:NO];
    
    [delegate zxingController:self didScanResult:twoDResult.text format: twoDResult.format];
}
...
{
    if ([self.delegate respondsToSelector:@selector(zxingController:shouldAutorotateToInterfaceOrientation:)]){
        return [self.delegate zxingController:self shouldAutorotateToInterfaceOrientation:interfaceOrientation];
    }
...
- (void) willAnimateRotationToInterfaceOrientation:(UIInterfaceOrientation)orientation duration:(NSTimeInterval)duration
{
    ...
    #if HAS_AVFF
    self.prevLayer.connection.videoOrientation = orientation;
    [self.prevLayer layoutSublayers];
    self.prevLayer.frame = self.view.bounds;
    #endif
